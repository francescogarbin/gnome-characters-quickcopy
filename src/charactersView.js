...
var CharactersView = GObject.registerClass({
    Signals: {
        'character-selected': { param_types: [GObject.TYPE_STRING, GObject.TYPE_BOOLEAN] },
    },
    Properties: {
    ...
    */
    ...
    onButtonPress(gesture, _nPress, x, y) {
        let hadj = this.get_hadjustment();
        let vadj = this.get_vadjustment();

        // Check if CTRL key is pressed
        let modifiers = gesture.get_current_event_state();
        this._ctrlPressed = (modifiers & Gdk.ModifierType.CONTROL_MASK) !== 0;
    ...
    onButtonRelease() {
        if (this._selectedCharacter)
            this.emit('character-selected', this._selectedCharacter, this._ctrlPressed);
    }
